package Ocean::Jingle::STUN::ServerComponent::RelayedTransport::AEHUDP;

use strict;
use warnings;

use parent 'Ocean::Jingle::STUN::ServerComponent::RelayedTransport';
...
sub _create_handle {
    ...
    my $handle = AnyEvent::Handle::UDP->new(
        ...
        on_error => sub {
            ...
            $self->{_delegate}->on_relayed_transport_error($fatal, $message);
        },
        on_recv => sub {
            my ($bytes, $h, $from) = @_;
            debugf('<UDP> @recv');
            ...
            $self->{_delegate}->on_relayed_transport_received_message($host, $port, $bytes);
        },
    );
}
...
sub start {
    ...
    $self->{_delegate}->on_relayed_transport_bound($self->{_host}, $self->{_port});
}
