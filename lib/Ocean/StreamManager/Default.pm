package Ocean::StreamManager::Default;
...
sub on_stream_bound_jid {
    ...
    unless (exists $self->{_bound}{$bound_jid->node}) {
        $self->{_bound}{$bound_jid->node} = {};
    }
    $self->{_bound}{$bound_jid->node}{$bound_jid->resource} = $stream;
...
sub on_stream_bound_closed {
    ...
    my $stream = delete $resources->{$bound_jid->resource};
...
sub on_stream_handle_sasl_password {
    ...
    $self->{_server}->on_stream_handle_sasl_password($stream_id, $username);
}

sub on_stream_handle_sasl_success_notification {
    my ($self, $stream_id, $username) = @_;
    $self->{_server}->on_stream_handle_sasl_success_notification($stream_id, $username);
}
...
sub on_server_completed_http_auth {
    ...
    my $stream = $self->find_stream_by_id($stream_id);
    $stream->on_server_completed_http_auth($user_id)
        if $stream;
}
...
sub on_server_delivered_sasl_password {
    ...
    my $stream = $self->find_stream_by_id($stream_id);
    $stream->on_server_delivered_sasl_password($password) if $stream;
}
